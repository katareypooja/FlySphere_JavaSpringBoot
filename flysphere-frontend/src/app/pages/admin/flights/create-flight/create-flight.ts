...
export class CreateFlight {
  ...
  generateFlightNumber() {
    ...
    // ✅ Generate highly unique number using timestamp (prevents collisions)
    const uniquePart = Date.now().toString().slice(-4);
    const randomPart = Math.floor(10 + Math.random() * 90); // 2-digit random
    const flightNo = `${prefix}${uniquePart}${randomPart}`;
  ...
  onSubmit() {
    ...
    this.http.post('http://localhost:8080/api/flights', payload)
      .subscribe({
        next: () => {
