...
export class FlightSearchComponent implements OnInit {
  ...
  setTripType(type: 'oneway' | 'round') {
    ...
    if (type === 'round') {
      ...
      // ✅ STRICT: When switching to round,
      // return date must ALWAYS be >= departure date
  ...
  set departureDate(value: string) {
    ...
    // ✅ Force return date to stay >= departure (ALWAYS)
    if (this._returnDate && this._returnDate < this._departureDate) {
      this._returnDate = this._departureDate;
      this.cdr.detectChanges();
    }
  ...
  set returnDate(value: string) {
    ...
    const depDate = this._departureDate ? new Date(this._departureDate) : null;
    const selectedDate = new Date(value);

    // ✅ STRICT: Only compare with departure date
    // Return date must NEVER be less than departure date
    if (depDate && selectedDate < depDate) {
      this._returnDate = this._departureDate;
      this.cdr.detectChanges();
      return;
    }
  ...
  private adjustReturnDate() {
    ...
    // ✅ If return date is empty → default it to departure date
    if (!this._returnDate) {
      this._returnDate = this._departureDate;
      return;
    }

    // ✅ STRICT: Return date must NEVER be less than departure date
    if (this._returnDate < this._departureDate) {
      this._returnDate = this._departureDate;
    }
  }
  ...
  searchFlights() {
    ...
    const departureUrl =
      `http://localhost:8080/api/flights?from=${fromCode}&to=${toCode}&date=${this.departureDate}`;
    ...
    this.http.get<any[]>(departureUrl).subscribe(data => {
      ...
      if (this.tripType === 'round' && this.returnDate) {
        ...
        const returnUrl =
          `http://localhost:8080/api/flights?from=${toCode}&to=${fromCode}&date=${this.returnDate}`;
        ...
        this.http.get<any[]>(returnUrl).subscribe(returnData => {
          ...
          this.returnFlights = returnData
            ...
            // ✅ Same-day return-after-arrival filtering
            .filter(returnFlight => {

              // Only apply when same date AND departure selected
              if (
                this.departureDate === this.returnDate &&
                this.selectedDeparture
              ) {
                const outboundArrival = this.selectedDeparture.arriveTime;
                const returnDeparture = returnFlight.departTime;

                if (!outboundArrival || !returnDeparture) return false;
